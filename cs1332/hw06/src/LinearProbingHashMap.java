...
public class LinearProbingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        if (key == null || value == null) {
            throw new IllegalArgumentException("cannot "
                    + "add null key or value to Hashmap");
        }
        if (((float) (size + 1)) / table.length > MAX_LOAD_FACTOR) {
            resizeBackingTable(2 * size + 1);
        }
        LinearProbingMapEntry current =
                new LinearProbingMapEntry(key, value);
        int hash = hashCompression(current);
        for (int i = 0; i < table.length; i++) {
            if (table[(hash + i) % table.length] == null) {
                table[(hash + i) % table.length] = current;
                size++;
                return null;

            } else if (table[(hash + i) % table.length].getKey()
                    == current.getKey()) {
                if (!table[(hash + i) % table.length].isRemoved()) {
                    V temp = table[(hash + i) % table.length].getValue();
                    table[(hash + i) % table.length]
                            .setValue((V) current.getValue());
                    return temp;

                } else {
                    table[(hash + i) % table.length]
                            .setValue((V) current.getValue());
                    table[(hash + i) % table.length].setRemoved(false);
                    size++;
                    return null;
                }
    ...
    /**
     * performs the hashcode and compression.
     * @param curr the entry being hashed
     * @return the compressed hash number
     */
    private int hashCompression(LinearProbingMapEntry curr) {
        int hash = curr.getKey().hashCode();
        hash =  java.lang.Math.abs(hash % table.length);
        return hash;
    }
    ...
    public V remove(K key) {
        if (key == null) {
            throw new IllegalArgumentException("Key cannot"
                    + " be removed because key is null");
        }
            for (LinearProbingMapEntry entry: table) {
    ...
    public void resizeBackingTable(int length) {
        if (length < size) {
            throw new IllegalArgumentException("length cannot be less "
                    + "than number of items in the hash map");
        }
        LinearProbingMapEntry<K, V>[] backingArray
                = new LinearProbingMapEntry[length];
        for (LinearProbingMapEntry element: table) {
            if (element != null && !element.isRemoved()) {
                int hash = hashCompression(element);
                for (int i = 0; i < table.length; i++) {
                    if (backingArray[(hash + i)
                            % backingArray.length] == null) {
                        backingArray[(hash + i)
